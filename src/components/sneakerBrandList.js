...
export default class SneakerBrandList extends React.Component{
    constructor(props){
        super(props)
        this.state ={
    ...
    render(){
        return(
        <>
        {this.state.sneakers.filter((sneaker) => sneaker.brand == this.props.brand).filter((sneaker) => sneaker.model.includes(this.props.search)).sort((s1, s2) => this.props.sort=='asc'? s1.price - s2.price: null).sort((s1, s2) => this.props.sort=='desc'? s2.price - s1.price: null).map((sneaker) => 
